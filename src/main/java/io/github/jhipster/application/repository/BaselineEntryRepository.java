...
public interface BaselineEntryRepository extends JpaRepository<BaselineEntry, Long> {
    @Query("select distinct baseline_entry from BaselineEntry baseline_entry left join fetch baseline_entry.baselineEntryVersions")
    List<BaselineEntry> findAllWithEagerRelationships();

    @Query("select baseline_entry from BaselineEntry baseline_entry left join fetch baseline_entry.baselineEntryVersions where baseline_entry.id =:id")
