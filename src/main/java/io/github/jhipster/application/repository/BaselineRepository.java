...
public interface BaselineRepository extends JpaRepository<Baseline, Long>, JpaSpecificationExecutor<Baseline> {
    @Query("select distinct baseline from Baseline baseline left join fetch baseline.fileGroups")
    List<Baseline> findAllWithEagerRelationships();

    @Query("select baseline from Baseline baseline left join fetch baseline.fileGroups where baseline.id =:id")
