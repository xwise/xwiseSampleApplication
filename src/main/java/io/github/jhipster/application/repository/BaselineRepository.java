...
public interface BaselineRepository extends JpaRepository<Baseline, Long> {
    @Query("select distinct baseline from Baseline baseline left join fetch baseline.baselines")
    List<Baseline> findAllWithEagerRelationships();

    @Query("select baseline from Baseline baseline left join fetch baseline.baselines where baseline.id =:id")
