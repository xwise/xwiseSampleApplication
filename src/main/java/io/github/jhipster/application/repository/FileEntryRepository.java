...
public interface FileEntryRepository extends JpaRepository<FileEntry, Long>, JpaSpecificationExecutor<FileEntry> {
    @Query("select distinct file_entry from FileEntry file_entry left join fetch file_entry.fileGroups")
    List<FileEntry> findAllWithEagerRelationships();

    @Query("select file_entry from FileEntry file_entry left join fetch file_entry.fileGroups where file_entry.id =:id")
