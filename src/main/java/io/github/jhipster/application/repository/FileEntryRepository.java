...
public interface FileEntryRepository extends JpaRepository<FileEntry, Long>, JpaSpecificationExecutor<FileEntry> {
    @Query("select distinct file_entry from FileEntry file_entry left join fetch file_entry.baselines left join fetch file_entry.uploadVersions")
    List<FileEntry> findAllWithEagerRelationships();

    @Query("select file_entry from FileEntry file_entry left join fetch file_entry.baselines left join fetch file_entry.uploadVersions where file_entry.id =:id")
