...
@Component({
    selector: 'jhi-baseline-anc-suffix',
    templateUrl: './baseline-anc-suffix.component.html'
})
export class BaselineAncSuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.baselineService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<BaselineAncSuffix[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/baseline-anc-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/baseline-anc-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: BaselineAncSuffix) {
        return item.id;
    }
